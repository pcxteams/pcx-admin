...
/**
 * Agent-facing render of an Office Page, built to the Agent Office print.
 *
 * The card styles are fixed; the arrangement is not. Sections fold into rows by
 * `layout.rowId` and size by `layout.span`, so the builder's layout drives this
 * output. `BuilderCanvas` draws its editing chrome over these same components,
 * which is what keeps the edit screen and the published page identical.
 *
...
function Brokerage({ section, data }: { section: OfficePageSection; data: OfficeRenderData }) {
  ...
  const hasBody = !!(
    b && (b.logoUrl || b.address || b.cityStateZip || b.phone || b.email || b.website)
  );

  return (
    <Panel>
      <Head icon={Building2} title={section.title || 'Brokerage Info'} />
      {!hasBody && <EmptyLine>Add your address and contact details in the Workspace Profile.</EmptyLine>}
      {hasBody && b && (
      ...
      <div className="grid grid-cols-1 gap-3 px-4 pb-3 sm:px-5 lg:grid-cols-[1.15fr_1fr]">
        <div className="relative min-h-[260px] overflow-hidden rounded-xl bg-gray-900">
          {featured.imageUrl ? (
            ...
            <div className="absolute inset-0 flex items-center justify-center text-white/10" aria-hidden>
            ...
            return (
              <li key={item.id}>
                ...
                >
                  ...
                  <ChevronRight size={15} className="mt-3 flex-none text-gray-300" strokeWidth={2} />
                </button>
              </li>
            );
          })}
  ...
  return (
    <Panel className="overflow-hidden">
      <div className="relative h-24 bg-gradient-to-br from-teal-600 to-teal-800">
        <div className="absolute inset-0 flex items-center justify-end pr-3 text-white/15" aria-hidden>
          <LifeBuoy size={88} />
        </div>
      ...
      <ul className="grid grid-cols-1 px-2 py-2 sm:px-3 sm:py-3">
        {items.map((item) => {
          ...
          const href =
            ...
            (member?.email ? `mailto:${member.email}` : member?.phone ? telHref(member.phone) : null);
          ...
          return (
            <li key={item.id}>
              ...
              >
                ...
                {href && <ArrowRight size={13} className="flex-none text-gray-300" strokeWidth={2.2} />}
              </Action>
            </li>
          );
        })}
...
export function OfficePageChrome({
  ...
  const hasVendorSection = content.sections.some(
    (s) => s.type === 'vendor-carousel' && s.visible
  );
