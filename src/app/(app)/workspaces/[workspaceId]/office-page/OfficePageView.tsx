...
import { groupSectionsIntoRows, SectionRow, TemplateStyles } from './builder/section-rows';

/**
 * Agent-facing render of a published Office Page — the "Office Access" portal.
 *
 * Presentational only (no hooks), so it backs BOTH the read/agent page and the
 * builder's live Preview toggle, keeping preview == published.
 *
 * Layout is driven by the builder's responsive row/column system: sections are
 * folded into rows by `layout.rowId` and sized by `layout.span` (via
 * `groupSectionsIntoRows` / `SectionRow`), exactly like the editor — so dragging
 * sections into columns and setting ratios in the builder changes this output.
 * Each section is then rendered with a designed, type-specific card style:
 *
...
function actionHref(action: OfficePageAction): string | null {
  ...
  if (action.type === 'phone') return dest.startsWith('tel:') ? dest : `tel:${dest.replace(/\s+/g, '')}`;
  return dest;
}
...
function activeItems<T extends { active: boolean; order: number }>(section: OfficePageSection): T[] {
    ...
    <section className="flex items-stretch justify-between gap-6 rounded-2xl border border-gray-200 bg-white px-7 py-6">
      <div className="flex flex-col justify-center">
        <h1 className="text-2xl leading-tight tracking-tight text-gray-900">
          {words.map((w, i) => (
            <span key={i} className={i === words.length - 1 ? 'font-extrabold italic' : 'font-light'}>
              {w}
              {i < words.length - 1 ? ' ' : ''}
            </span>
          ))}
      ...
      >
        ...
        {brandLabel && (
          <div className="absolute inset-0 flex flex-col items-end justify-center pr-4 text-right">
            <span className="text-sm font-bold uppercase tracking-widest text-white">{brandLabel}</span>
          </div>
        )}
      </div>
    </section>
...
function Announcements({ section }: { section: OfficePageSection }) {
  ...
  return (
    <section>
      <SectionHeader label={section.title || 'Office Announcements'} action={{ label: 'View All' }} />
...
function ImportantResources({ section }: { section: OfficePageSection }) {
  ...
  return (
    <section>
      ...
      <a href="#" className="mt-2.5 inline-flex items-center gap-0.5 text-xs font-medium text-blue-600 hover:text-blue-700">
        View All Resources
        <ChevronRight size={13} />
      </a>
    </section>
...
function Events({ section }: { section: OfficePageSection }) {
  ...
  return (
    <section>
      <SectionHeader label={section.title || 'Upcoming Events'} action={{ label: 'View Calendar' }} />
      <div className="rounded-xl border border-gray-200 bg-white">
        {items.map((e, i) => {
...
function Contacts({ section }: { section: OfficePageSection }) {
  ...
  return (
    <section>
          ...
          return (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                {c.description && <p className="truncate text-[11px] text-gray-400">{c.description}</p>}
                {phone && <p className="truncate text-[11px] text-gray-400">{phone}</p>}
              </div>
              {href && (
                <a href={href} className="shrink-0 text-gray-300 hover:text-blue-600" aria-label={`Contact ${c.title}`}>
                  <RightIcon size={14} />
                </a>
              )}
            </div>
      ...
      <a href="#" className="mt-2 inline-block text-xs font-medium text-blue-600 hover:text-blue-700">
        View All Contacts
      </a>
    </section>
  );
...
function Footer({ footer }: { footer?: OfficePageContent['footer'] }) {
  ...
  const cells: { icon: ComponentType<{ size?: number; className?: string }>; label: string; value: ReactNode }[] = [];
  if (footer.officeHours) cells.push({ icon: Clock, label: 'Office Hours', value: footer.officeHours });
  if (footer.website)
    cells.push({
      icon: Globe,
      label: 'Company Website',
      value: footer.websiteUrl ? (
        <a href={footer.websiteUrl} target="_blank" rel="noopener noreferrer" className="hover:text-blue-600">
          {footer.website}
        </a>
      ) : (
        footer.website
  ...
  return (
    <section>
      ...
      <div className="grid gap-3 [grid-template-columns:repeat(auto-fit,minmax(200px,1fr))]">
        {items.map((l) => (
          <div key={l.id} className="flex items-center gap-3 rounded-xl border border-gray-200 bg-white px-4 py-3">
            <span
...
function SectionBlock({
  ...
  switch (section.type) {
    ...
    case 'quick-links':
      return isShortcuts(section) ? <Shortcuts section={section} /> : <QuickTools section={section} />;
    case 'resources':
...
export default function OfficePageView({ content }: { content: OfficePageContent }) {
  ...
  // Fold sections into rows exactly like the builder, so the editor's row/column
